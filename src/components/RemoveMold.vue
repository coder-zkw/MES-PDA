...
export default {
    ...
    methods: {
        // 获取品检完的工单列表
        saveRemoveMold() {
            // axios.post('http://localhost:50814/MES/SaveMouldUnload', {
        ...
        scaning() {
            ...
            // axios.get('http://localhost:50814/MES/GetMouldUnload?md=' + this.scanVal).then(res => {
                // console.log(res)
                if(res.status === 200){
                    if (res.data.length === 0) {
                        return this.$toast.fail('该模具不存在')
                    } else if (res.data[0].STATUS !== 15) {
                        return this.$toast.fail('该模具不在下模列表')
                    }
