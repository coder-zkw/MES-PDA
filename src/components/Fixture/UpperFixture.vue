<template>
    <div class="upperFixture">
        <van-nav-bar fixed title="上治具" left-text="返回" left-arrow @click-left="$router.back()" />
...
export default {
    ...
    methods: {
        SaveMouldLoad() {
            // axios.post('http://localhost:50814/MES/SaveFixtureLoad', {
            axios.post(this.httpUrl + 'MES/SaveFixtureLoad', {
                EM_FIXTURE: this.mold,
                EM_USER: localStorage.getItem('userName')
            }).then(res => {
                // console.log(res)
                if(res.data.code === 200){
                    this.$toast.success('上治具成功')
                    this.resetScan()
                }else{
                    this.$toast.fail('上治具失败')
                }
                this.scanVal = ''
            }).catch(err => err)
        },
        scaning() {
            ...
            if (this.mold === '') {
                ...
                axios.post(this.httpUrl + 'MES/getUpFixutre', {
                // axios.post('http://localhost:50814/MES/getUpFixutre', {
                    EM_MOULD: currenMold
                }).then(res => {
                    // console.log(res)
                    var result = res.data
